...
function newLine()
{
if(document.getElementById('minigameCheck').checked)
  {
    const biology = ["Flora", "Fauna", "Stad, Dorp of Land", "Een huisdier", "Iets uit de zee", "Bij de dierentuin", "Een god of godin", "Bij de groenteboer", "Iets scheikundigs", "Iets natuurkundigs", "Iets biologisch", "Een sportclub", "Een huishoudelijk product", "Een kleur", "Iets duurs"];
    const people = ["Een mannennaam", "Een vrouwennaam", "Bekend persoon", "Een fictief persoon", "Een artiest", "Een liedje", "Bekende nederlander", "Snoep/Chips", "Drinken alcohol", "Drinken non-alcohol", "Basketbalspeler", "Voetbalspeler","Merk kleding of auto", "Een achternaam", "Iets kleins"];
    const random = ["Iets waarmee je iemand kan vermoorden", "Een woord wat je niet kent maar wel denkt dat het iets is", "Iets waardoor je van je fiets kan vallen", "Wapen voor zombie apocalyps (moet goedgekeurd worden)", "Buitenlands woord (geen engels)", "Iets wat je doodt als het op je valt", "Iets wat je bij de bouwmarkt kan kopen", "een technologische term", "Iets aardrijkskundig" , "Iets brandbaars", "Iets wiskundigs", "Iets sportgerelateerds", "Iets dat kinderen buiten doen", "Iets dat je in de kroeg kan vinden", "Iets dat je op vakantie kan doen", "Iets wat je graag in huis hebt"];
    const games = ["Pokemon", "Ninetendo gerelateerd", "Films", "Digitaal spel", "Een app op je telefoon", "Analoog spel", "Iets uit brawl stars", "Een elektronisch object", "Een computerterm", "een voertuig", "Iets uit de geschiedenis", "Een baan", "Iets groots", "Iets dat je in je portomonee kan vinden", "Iets dat je kan vervormen met je handen", "Iets uit Minecraft"];
    const jobs = ["Iets uit de mythologie", "Iets dat in je zak past", "Iets sportgerelateerd", "Iets wat je bij een restaurant zou kunnen halen", "Iets wat je op school kan vinden", "Iets brandbaars", "Iets bij de slager", "iets dat je op straat kan vinden", "iets verslavends", "Iets in je lichaam", "Een water of eiland", "Iets goedkoops", "Iets dat in je zak past", "Een monster" , "Iets dat je liever niet in huis hebt"];
  ...
  else
  {
    ...
    const categories = [biology, people, random, games, jobs, minigames];
    let pickCategories = Math.floor(Math.random() * categories.length);
    let pickThemes = Math.floor(Math.random() * categories[pickCategories].length);
    document.getElementById("text").innerHTML = categories[pickCategories][pickThemes];
  }
